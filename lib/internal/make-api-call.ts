...
exports.inject = function(options: any) {
   ...
   * }}
   */
  return function(path: string, query: any, callback: Function) {
    ...
    var canRetry = queryOptions.canRetry || function(response: any, query: any) {
      return (
        response == null
        || response.status === 500
        || response.status === 503
    ...
    var isSuccessful = queryOptions.isSuccessful || function(response: any) {
    ...
    function rateLimitedGet() {
      return requestQueue.add(function() {
        return Task.start(function(resolve: Function, reject: Function) {
    ...
    var requestTask = attempt({
      'do': rateLimitedGet,
      until: function(response: any) { return !canRetry(response, query); },
    ...
    var task =
        ...
        .thenDo(
            function(response: any) { callback(null, response); },
            function(err: Error) { callback(err); });

    if (options.Promise) {
      var originalCallback = callback;
      var promise = new options.Promise(function(resolve: Function, reject: Function) {
        callback = function(err: Error, result: any) {
          if (err != null) {
            reject(err);
          } else {
            resolve(result);
   ...
   */
  function formatRequestUrl(path: string, query: any, useClientId: boolean) {
    if (channel) {
   ...
   * @return {string}
   */
  function computeSignature(secret: Buffer, payload: string) {
    var signature =
        new Buffer(
            require('crypto')
