 ...
 * @template T
 */
Task.start = function(doSomething: Function) {
  var me: any = {};
  ...
  function finish(err: string|null, result: any) {
  if (!finished) {
  ...
  try {
    ...
    var abort = doSomething(
        function(result: any) { finish(null, result); },
        function(err: string)    { finish(err,  null);   });
  } catch (err) {
    finish(err, null);
  }
   ...
   */
  function setListener(callback: Function) {
    if (onFinish) {
   ...
   * @template U
   */
  me.thenDo = function(onResolve: Function, onReject: Function) {
   ...
   * @return {THIS}
   */
  me.finally = function(cleanup: Function) {
    if (!finished) {
 ...
 */
Task.withValue = function(result: any) {
  return Task.start(function(resolve: Function) {
    resolve(result);
  });
};
 ...
 */
Task.withError = function(err: Error) {
 ...
 * @template T, U
 */
function compose(firstTask: any, whenFirstTaskFinishes: Function, onResolve: Function, onReject: Function) {
  return Task.start(function(resolve: Function, reject: Function) {
    var cancelled: boolean;
    var currentTask = firstTask;

    whenFirstTaskFinishes(function(err: string, result: any) {
      currentTask = null;
