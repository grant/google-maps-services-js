...
export const inject = (wait: Function) => {
  return {
    /**
     ...
     */
    attempt: (options: any) => {
      const doSomething = options['do'];
      const isSuccessful = options.until;
      let interval = options.interval || 500;
      const increment = options.increment || 1.5;
      const jitter = options.jitter || 0.5;

      return Task.withValue().thenDo(function loop() {
        return doSomething().thenDo((result: any) => {
          if (isSuccessful(result)) {
            return Task.withValue(result);
          }

          const delay = interval * (1 + jitter * (2 * Math.random() - 1));
          interval *= increment;
